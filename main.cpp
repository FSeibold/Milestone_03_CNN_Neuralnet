...
std::vector<std::vector<int>> Channel::activation()
{
    for(int i = 0; i < imageM.size(); i++){
        for(int j = 0; j < imageM.size(); j++){
...
void Channel::do_Kernel2()
{
    ...
    for (size_t i = 0; i < featureMap.size(); ++i)
    {
        ...
        {
            featureMap[i][j] = maxPooling(transition(imageM, (kernelM.size() - 1) * i, (kernelM[0].size() - 1) * j,
...
int Channel::maxPooling(std::vector<std::vector<int>> imageM)
{
    int s = 0;
    // Test:
